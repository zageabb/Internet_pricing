...
def load_page(url, max_age_days=14, allow_stale=False, path=DEFAULT_CACHE):
    if not Path(path).exists():
        return None
    with closing(connect(path)) as connection:
        row = connection.execute("SELECT fetched_at, payload_json FROM pages WHERE url = ?", (url,)).fetchone()
    if not row:
        return None
    try:
...
def store_page(url, payload, path=DEFAULT_CACHE):
    if not payload.get("text"):
        return
    stored = {key: payload.get(key, "") for key in ("text", "url", "content_type", "published_at")}
    fetched_at = datetime.now(timezone.utc).isoformat(timespec="seconds")
    with LOCK, closing(connect(path)) as connection:
        with connection:
            connection.execute("""
                INSERT INTO pages (url, fetched_at, payload_json) VALUES (?, ?, ?)
                ON CONFLICT(url) DO UPDATE SET fetched_at=excluded.fetched_at, payload_json=excluded.payload_json
